...
import { LayoutDashboard, Truck, Users, MapPin, Wrench, FileBarChart, LogOut } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className="w-64 bg-slate-900 text-white flex flex-col fixed h-screen">
        <div className="p-6 border-b border-slate-800">
          ...
          <p className="text-xs text-slate-400 mt-1">Fleet Operations</p>
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          ...
          <p className="text-xs text-slate-400 mb-3">{user?.role?.replace("_", " ")}</p>
          ...
          {navItems.map((item) => {
            ...
            return (
              <a
                key={item.href}
                href={item.href}
                className={`flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm transition ${active ? "bg-emerald-600 text-white" : "text-slate-300 hover:bg-slate-800"}`}
              >
                <Icon size={18} />
                {item.name}
              </a>
